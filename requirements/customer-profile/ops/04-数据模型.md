# 04 数据模型 — 运营画像模块（customer-profile-ops）

> 模块：customer-profile-ops（运营画像，**客户画像二期**） ｜ 关联 PRD 母本：`plans/customer-profile/ops/customer-profile-ops-prd.md`（v1.3 已定版） ｜ 状态：**二期冻结基线**（2026-08-26 roadmap 修订：表名随模块改名同步，`ai_customer_profile` → `ai_customer_profile_ops`）
>
> **DDL 边界（Q20）**：仅新增 `ai_customer_profile_ops` 一张表；不与 scheduled-task 共表；不改动既有任何表。画像存储不含指标快照（Q21：v2 全量保留时引入）。
>
> **存储形态（Q39，r4）**：AI 分析输出**整包存 `profile_json`**（原生 json 类型）——指标后续新增/变更零 DDL；仅 FR-06 五个筛选键设检索投影列（见「投影与一致性」）。

## 1. 新表 `ai_customer_profile_ops`（运营画像，v1 覆盖式）

```sql
CREATE TABLE `ai_customer_profile_ops` (
...
Q21 显式决策：v1 覆盖式只留最新，指标快照随 v2"全量保留 + is_latest"一并引入。届时新增历史表（如 `ai_customer_profile_ops_history`）而非在本表加列，避免热表膨胀。

## 2. 与既有表的关系

```mermaid
erDiagram
    ai_customer_profile_ops ||..|| MCP_CUSTOMER : "customer_id 引用（逻辑外键，跨服务）"
    ai_customer_profile_ops }o--|| AGENT : "生成经由（百炼，无落库关系）"

    ai_customer_profile_ops {
        bigint id PK
