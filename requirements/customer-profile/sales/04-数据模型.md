...
> 模块：customer-profile-sales（销售转化画像） ｜ 关联 PRD 母本：`plans/customer-profile/customer-profile-sales-prd.md`（v1.1 已定版） ｜ 状态：已定版
>
> **DDL 边界（Q8）**：仅新增 `ai_customer_profile_sales` 一张表；不与 ops 的 `ai_customer_profile` 共表；不改动既有任何表。
>
> **存储形态（Q21，对齐 ops Q39 整包模式）**：AI 分析输出**整包存 `profile_json`**（原生 json 类型）——指标后续新增/变更零 DDL；四个筛选键（Q7/Q24）设检索投影列（见「投影与一致性」）。

## 1. 新表 `ai_customer_profile_sales`（销售画像，v1 覆盖式）

```sql
CREATE TABLE `ai_customer_profile_sales` (
    ...
    `skill_version`    varchar(32)   DEFAULT NULL COMMENT '产出本画像的销售画像技能版本（应用配置 profile.sales.skill-version 注入）',
...
| `skill_version` | 产出画像的技能版本，应用配置注入（服务不解析镜像，FR-07.3 不破） | Q16 |
...
| `idx_filter` | 四筛选键中三枚举的组合索引；`info_completeness` 为 tinyint 精确匹配，单列选择性低不单独设索引（组合 v2 视查询表现补） | Q24 |
| `idx_upd` | 列表次序排序（generated_time 降序以 `upd_time` 承载，成功分析必然更新本行）+ 僵死复位轮询扫描 | FR-05.2/NFR-03 |

### 与 ops 表的差异

| 维度 | `ai_customer_profile`（ops） | `ai_customer_profile_sales`（本表） |
|---|---|---|
...
```mermaid
erDiagram
    ai_customer_profile_sales ||..|| MCP_CUSTOMER : "customer_id 引用（逻辑外键，跨服务）"
    ai_customer_profile_sales ||..|| AI_CUSTOMER_PROFILE : "同客户两套画像（独立表，无关联）"

    ai_customer_profile_sales {
        bigint id PK
...
- **零改动**：`ai_customer_profile`（ops）/ `ai_conversation` / `ai_scheduled_task` 等均不动——两套画像**不共表、不建物理关联**（Q23 独立演进；同 `customer_id` 仅逻辑对应，由 MCP 侧标识体系保证一致）
