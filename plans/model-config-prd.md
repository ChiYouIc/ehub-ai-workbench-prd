...
15. **v1 交付范围（Q7/Q25）**：仅管理界面 + 配置存储 + CRUD 接口。不实现任何第三方模型的调用代码。自研 Agent 对话规划为**独立模块 agent-chat（v2）**，不并入本模块
...
### v1 范围外
- 自研 Agent 对话（规划为独立模块 agent-chat，v2 启动）
- 第三方模型的实际调用代码（Provider 适配器，随 agent-chat 模块交付）
- 第三方模型的对话接口（随 agent-chat 模块交付）
