...
- **到点自动执行**：服务内 `@Scheduled` 轮询任务表，到期任务原子占位后以非流式方式调用百炼
...
**总体策略**：在现有 chat/conversation 双域之外新增 `task` 领域；调度为服务内单实例 DB 轮询，执行复用百炼 SDK 非流式调用，产出完全复用既有会话/内容表，仅新增一张任务表。

### 调度与执行

1. **调度基础设施（Q1）**：Spring `@Scheduled` 固定延迟轮询 DB 任务表（`taskPollFixedDelay`，默认 30s），单实例部署前提；未来多实例需换 Quartz/分布式锁（演进项，见 Out of Scope 记录）
2. **触发条件（Q9）**：轮询扫描 `enabled=1 AND is_del=0 AND next_fire_time <= now` 的任务，错过的多个触发点**合并为一次执行**；执行后 `next_fire_time` 推进为 cron 的下一个未来触发点
3. **防重入占位（Q11）**：触发用原子 UPDATE 占位：`SET run_state='RUNNING', next_fire_time=<cron下一未来点> WHERE id=? AND run_state='IDLE'`，影响行数=0 即跳过（天然串行，无需锁表）；调度轮询单线程，执行提交到独立任务线程池（core 2 / max 4），池满拒绝时记日志、复位 IDLE、该次触发放弃（`next_fire_time` 已推进，接受跳过）
...
5. **僵死复位（Q11）**：`run_state=RUNNING` 超过 `runningStaleMinutes`（默认 30 分钟，可配）视为进程被杀残留，轮询时复位 IDLE；复位不触发补跑（等下一个 `next_fire_time`）
6. **时钟（Q19）**：统一注入 `Clock` bean（生产系统时区，测试固定时钟）；cron 按 JVM 本地时区解释，不做用户级时区转换
...
2. **调度触发单测（mock mapper + 固定 Clock）**：到期触发并推进 next_fire_time；占位 UPDATE 0 行跳过；停用/已删不被扫描；RUNNING 超 30 分钟僵死复位
...
- 多实例部署的分布式调度（Quartz / 分布式锁 / xxl-job）——单实例 DB 轮询为 v1 前提
...
- **演进方向**：多实例调度、执行历史流水、失败重试策略、任务模板、通知推送
