...
> 状态：**已定版 v1.0（终审通过）** — 2026-08-22
> 决策记录：Q1–Q21 已于 2026-08-21 两轮访谈确认（用户采纳全部推荐答案），2026-08-22 终审通过，定版 v1.0。
> 终审修订 r2：部署形态更正为**多实例共库**（推翻 Q1 单实例前提）——互斥机制改为 DB 行级原子抢占，跨实例同一任务至多一个执行（详见「调度与执行」1/3/5/6）。
> 终审修订 r3：多实例负载分布**不保证均匀**定为 v1 接受边界（退化信号=「池满放弃触发」日志频率）；均匀分布优化、执行历史流水表（另扩展表）、执行看板列入 **v2 计划**——看板数据作为是否进一步优化执行策略的评估依据。
> 终审确认：r3 随终稿一并通过；残留小决策三项确认采纳（见 Further Notes）。
...
- 多实例负载分布**不保证均匀**（r3 定案）：抢占为赛跑语义——忙实例不让位、闲实例不加成，轮询相位锁死或 DB 往返稳定差异可致长期偏斜。v1 量级下执行成本主体为百炼调用（按 appKey 全局计费，与哪个实例执行无关），均匀性无实际收益，接受；退化信号=**「池满放弃触发」日志频率上升**，出现即触发 v2 执行策略评估
...
- 独立调度中心（Qua（仅任务表记最近一次结局；是否引入重试随 v2 看板数据评估）
- 执行历史流水表与执行看板（**v2 计划**，r3 排定：另扩展表逐轮记录 → 看板分析 → 据数据评估执行策略）
- 多实例负载均匀分布优化——随机相位错峰、容量感知抢占等（**v2 视看板数据决定**；v1 接受赛跑语义偏斜，见「已知边界」 分布式锁组件）——DB 行级原子抢占已满足多实例互斥，不引入
...
- **v2 计划（r3 排定）**：执行历史流水表（另扩展表，逐轮记录执行实例/结局/耗时）→ 执行看板（实例分布偏斜度、池满放弃触发频率、触发延迟、成功率与失败原因分布）→ **依据看板数据评估**是否优化执行策略（随机相位、容量感知抢占，乃至更均匀的分配机制）及失败重试策略
- **演进方向（未排期）**：m` 无 `PARAM_ERROR`（既有缺口，与 chat-conversation PRD 共用）；MyBatis-Plus 分页插件（同一前置）；本功能新增 `SCHEDULED(6)` 枚举值 + `Clock` bean + 任务线程池
- **与既有模块关系**：产出落库复用 `ai_conversation`/`ai_conversation_content` 与会话内容查询接口；不依赖 v1 SSE 流式链路（非流式独立调用）
- **残留小决策（终审已确认采纳）**：SCHEDULED 枚举数值取 6；绑定会话组 name=任务名；删会话组后任务继续执行的已知边界
