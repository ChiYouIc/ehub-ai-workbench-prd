...
- **销售画像**是面向销售转化的一次分析产出：评分面板（质量/热度/优先级 AI 判断 + 完整度规则计算）+ 速读 + 转化策略 + 跟进话术。与运营画像（分层/生命周期/流失风险）同客户不同视角，独立生成、独立存储。
- **数据工具**是画像的数据获取层：下游 MCP 统计服务暴露的按业务问题切分的查询工具（约 7~8 个，带时间窗口参数，回溯上限 365 天）。统计口径归数据属主，本系统不拥有。
- **画像技能**是画像的分析规则载体：规定数据获取策略、分析维度判定标准和输出结构。其单一事实源在本仓库，运行时配置于百炼 Agent 侧。
- **指标包**是逻辑概念：画像所需指标全集，由数据工具结果按画像技能规定的序列拼装而成。
- **画像**是一次分析的产出：固定 schema 的结构化标签（价值分层/生命周期/流失风险/主营品类/经营特征/建议动作）+ 中文经营摘要。v1 覆盖式只留最新。
