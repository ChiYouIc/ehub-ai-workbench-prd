...
│   └── customer-profile/  #   用户画像域（新用户转化；sales/ops 两旧模块已于 2026-08-27 删除，git 历史可溯）
│       ├── customer-profile-prd.md #   用户画像 PRD（v1.0 草案待审）
│       └── customer-profile-interview.md # Q1–Q28 溯源（含废弃决策 Q1/Q7/Q23）
...
| 用户画像（新用户转化）v1 | `plans/customer-profile/customer-profile-prd.md`（v1.0 草案待审）+ `customer-profile-interview.md`（Q1–Q28） | —（待拆解，将落 `requirements/customer-profile/` 根 01–05） | — | —（卡片 ia 设计待办，沿用业务方七区块线框） | **PRD 草案待审**；取代已删除的 sales/ops 两模块（2026-08-26 废弃，2026-08-27 删除）；前端由宿主业务系统承载；下游 MCP 仅 1 数据工具 |
